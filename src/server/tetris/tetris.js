...
function handleGame(game, player, socket) {
  if (player.needsUpdate) {
    ...
    draw(
      player.map,
      player.currentPieceX,
      player.currentPieceY,
      game.pieces[player.currentPiece].content[player.currentPieceRotation],
      eraseLine
    );
    erasePieceSpecter(
      player,
  ...
  if (
    new Date().getTime() - player.pieceTimestamp < player.gravityInterval &&
    !player.needNewPiece &&
  ...
  if (game.pieces.length - player.currentPiece < 10) game.addPieces(10);

  if (player.needNewPiece) player.getNextPiece();
}
...
function handleGravity(player, piece) {
  ...
  if (
    ...
    for (let i = 0; i < pieceContent.length; i++) {
      if (player.currentPieceY + i >= 0) break;
    ...
    // If the piece is not at the bottom and lock delay was set, remove it
      ...
      player.gravityInterval == 500 &&
      new Date().getTime() - player.pieceTimestamp >= 500
    ) {
      player.gravityInterval = consts.levels[player.level - 1];
    }

    //   new Date().getTime() - player.pieceTimestamp >=
    //   player.gravityInterval
    // ) {
    // }
  }
}
...
function handleClearedLines(player) {
  ...
  if (player.clearedLines / 10 > player.level) player.increaseLevel();
